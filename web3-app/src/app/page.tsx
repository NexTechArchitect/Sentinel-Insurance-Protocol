...
function orbitVector(p: Planet, time: number) {
  ...
  const xO = r * Math.cos(angle), zO = r * Math.sin(angle);
...
async function makeLabelTextureAsync(text: string, color: string): Promise<THREE.Texture> {
  ...
  ctx.closePath();
  ctx.fillStyle = 'rgba(0,3,12,0.82)'; ctx.fill();
...
function CinematicScene() {
  ...
  useEffect(() => {
    ...
    const rl = new THREE.PointLight(0xffc676, 3, 800); rl.position.set(0, -130, -300); scene.add(rl);
    const satL = new THREE.PointLight(0x88ccff, 6, 300); satL.position.set(-60, 80, 150); scene.add(satL);

    const root = new THREE.Group();
    // FIX: respect mobile offset at init
    ...
    starLayers.forEach(({ n, rMin, rMax, sz, op, spd }) => {
      ...
      for (let i = 0; i < n; i++) {
        const theta = Math.random() * Math.PI * 2, phi = Math.acos(2 * Math.random() - 1), r = rand(rMin, rMax);
        pos[i*3] = r*Math.sin(phi)*Math.cos(theta); pos[i*3+1] = r*Math.cos(phi)*0.44; pos[i*3+2] = r*Math.sin(phi)*Math.sin(theta);
        const c = sPalette[Math.floor(Math.random() * 4)], b = 0.45 + Math.random() * 0.55;
    ...
    const nebulaGroup = new THREE.Group(); root.add(nebulaGroup);
    {
      ...
      for (let i = 0; i < n; i++) {
        const r = 80 + Math.pow(Math.random(), 1.8) * 260, theta = Math.random() * Math.PI * 2, phi = Math.acos(2 * Math.random() - 1);
        pos[i*3] = r*Math.sin(phi)*Math.cos(theta); pos[i*3+1] = r*Math.cos(phi)*0.28; pos[i*3+2] = r*Math.sin(phi)*Math.sin(theta);
        const c = Math.random() < 0.6 ? new THREE.Color('#58daff') : new THREE.Color('#c484ff');
    ...
    const saturnBody = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ map: sTex, emissiveMap: sTex, emissive: new THREE.Color('#1a4a7a'), emissiveIntensity: 0.45, metalness: 0.05, roughness: 0.62 })
    );
    ...
    // Rings
    const makeRingTex = (col: string, op: number) => {
      const rc = document.createElement('canvas'); rc.width = 512; rc.height = 1;
      const rCtx = rc.getContext('2d')!;
      const rg = rCtx.createLinearGradient(0, 0, 512, 0);
      const c = new THREE.Color(col);
      const r = Math.round(c.r*255), g2 = Math.round(c.g*255), b = Math.round(c.b*255);
      rg.addColorStop(0,    `rgba(${r},${g2},${b},0)`);
      rg.addColorStop(0.05, `rgba(${r},${g2},${b},${op})`);
      rg.addColorStop(0.95, `rgba(${r},${g2},${b},${op * 0.7})`);
    ...
    PLANETS.forEach(p => {
      ...
      for (let i = 0; i < n; i++) { const a = (i/n)*Math.PI*2+rand(-0.05,0.05), r = p.orbit+rand(-5,5), xO = r*Math.cos(a), zO = r*Math.sin(a), cosI = Math.cos(p.inc), sinI = Math.sin(p.inc), cosL = Math.cos(p.lan), sinL = Math.sin(p.lan); pos[i*3]=cosL*xO-sinL*zO*cosI+rand(-2,2); pos[i*3+1]=zO*sinI+rand(-1.5,1.5); pos[i*3+2]=sinL*xO+cosL*zO*cosI+rand(-2,2); }
    ...
    // Ambient dust
    {
      ...
      for (let i = 0; i < n; i++) { const r=rand(260,1300),theta=Math.random()*Math.PI*2,phi=Math.acos(2*Math.random()-1); pos[i*3]=r*Math.sin(phi)*Math.cos(theta); pos[i*3+1]=r*Math.cos(phi)*0.36; pos[i*3+2]=r*Math.sin(phi)*Math.sin(theta); const c=Math.random()<0.55?new THREE.Color('#58daff'):new THREE.Color('#c484ff'); col[i*3]=c.r*0.38;col[i*3+1]=c.g*0.38;col[i*3+2]=c.b*0.38; }
    ...
    const comets: Comet[] = Array.from({ length: 6 }, (_, i) => {
      const arr = new Float32Array(6), geo = new THREE.BufferGeometry();
      geo.setAttribute('position', new THREE.BufferAttribute(arr, 3));
      const line = new THREE.Line(geo, new THREE.LineBasicMaterial({ color: cometColors[i % 4], transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false }));
      scene.add(line);
    ...
    // FIX: root.position.x updated correctly on resize
    const onR = () => {
      renderer.setPixelRatio(Math.min(window.devicePixelRatio ?? 1, 2.5));
      renderer.setSize(window.innerWidth, window.innerHeight);
    ...
    const tick = () => {
      ...
      auroraLayers.forEach(({ pts, buf, meta }) => {
        meta.forEach((m, i) => { const a = m.theta + t*m.spd*0.28, wave = Math.sin(t*0.20 + m.phase)*20; buf[i*3]=Math.cos(a)*m.r; buf[i*3+1]=m.y+wave; buf[i*3+2]=Math.sin(a)*m.r; });
      ...
      orbStates.forEach((state, idx) => {
        const p = state.planet, pos = orbitVector(p, t);
        const interpPos = pos.clone().lerp(new THREE.Vector3(0,0,0), hackLevel * 0.5);
        if (recoveryLevel > 0.02) interpPos.add(pos.clone().normalize().multiplyScalar(recoveryLevel * 24 * Math.sin(t * 2 + idx)));
        ...
        if (idx < 4) { orbLights[idx].position.copy(interpPos); orbLights[idx].intensity = 2 + pulse * 0.6; }
      });
      comets.forEach(c => {
        const prog = (t * c.spd + c.seed) % 1;
        const x = -750 + prog * 1500;
...
function StatChip({ label, value, accent, index }: { label: string; value?: string; accent: string; index: number }) {
  return (
    <div className="ss-stat-chip" style={{ '--accent': accent, '--offset': `${(index - 2) * 3}px` } as React.CSSProperties}>
      <i /><span>{label}</span><strong>{value ?? 'SYNCING'}</strong>
    </div>
  );
}
...
function SectionTitle({ label, title, body }: { label: string; title: string; body?: string }) {
  return (
    <div className="ss-section-title">
      <span>{label}</span><h2>{title}</h2>{body && <p>{body}</p>}
    </div>
  );
}
...
function ActionCard({ href, label, title, desc, color }: { href: string; label: string; title: string; desc: string; color: string }) {
  return (
    <Link href={href} className="ss-command-card" style={{ '--accent': color } as React.CSSProperties}>
      <span>{label}</span><strong>{title}</strong><p>{desc}</p><em>Launch →</em>
    </Link>
  );
}
...
export default function DashboardPage() {
  ...
  // KEY FIX: isMobile starts false; we set BOTH isMobile and mounted together
  // so the early-return gate ensures CinematicScene never mounts on mobile
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    // FIX: determine mobile FIRST, then flip mounted — so the !mounted early
    // return holds until isMobile is already correct. This prevents the brief
    // window where isMobile=false caused THREE.js to mount on mobile.
    const mobile = window.innerWidth < 860 || ('ontouchstart' in window);
    setIsMobile(mobile);
    setMounted(true);

    const check = () => setIsMobile(window.innerWidth < 860 || ('ontouchstart' in window));
  ...
  const heroOpacity = isMobile ? 1 : Math.max(0, 1 - scrollY / 600);
  const heroY       = isMobile ? 0 : scrollY * 0.12;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* ── Mobile gradient background (replaces WebGL) ── */
        .ss-mobile-bg {
          display: none;
          position: fixed; inset: 0; z-index: 0; pointer-events: none;
          background:
            radial-gradient(ellipse 130% 55% at 50% -5%, rgba(25,45,110,0.65), transparent 55%),
            radial-gradient(ellipse 80%  50% at 15% 65%, rgba(55,10,95,0.45),  transparent 55%),
            radial-gradient(ellipse 70%  40% at 85% 85%, rgba(5,40,30,0.35),   transparent 52%),
            #010208;
        }
        @media (max-width: 859px) { .ss-mobile-bg { display: block; } }
        ...
        .ss-ham { background: none; border: none; cursor: pointer; padding: 6px; display: none; flex-direction: column; gap: 5px; align-items: center; justify-content: center; }
        @media (max-width: 860px) {
          .ss-nav-links { display: none !important; }
          .ss-wallet-chip { display: none !important; }
          .ss-ham { display: flex !important; }
          .ss-nav { padding: 12px 16px; }
        }

        /* ── Mobile menu ── */
        .ss-mobile-menu { position: fixed; inset: 0; z-index: 850; background: rgba(1,2,8,0.97); backdrop-filter: blur(28px); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 28px; pointer-events: none; opacity: 0; transition: opacity 0.3s; }
        .ss-mobile-menu.open { pointer-events: auto; opacity: 1; }
        .ss-mobile-link { font-family: "Cormorant Garamond", serif; font-size: clamp(28px,8vw,42px); font-style: italic; font-weight: 700; color: #fff; text-decoration: none; opacity: 0; transform: translateY(14px); transition: all 0.4s cubic-bezier(0.16,1,0.3,1); }
        .ss-mobile-menu.open .ss-mobile-link { opacity: 1; transform: none; }
        .ss-mobile-link:hover { color: #58daff; }
        .ss-mobile-close { position: absolute; top: 20px; right: 20px; background: none; border: none; color: rgba(255,255,255,0.4); font-size: 28px; cursor: pointer; padding: 8px; }
        ...
        /* FIX: mobile hero — no empty space, content fills naturally */
        @media (max-width: 860px) {
          .ss-hero {
            min-height: unset;
            justify-content: flex-start;
            align-items: flex-start;
            padding: clamp(76px,18vw,100px) clamp(16px,5vw,24px) clamp(36px,8vw,52px);
          }
        }
        .ss-hero-copy { width: min(500px, 100%); will-change: opacity, transform; }
        @media (max-width: 860px) { .ss-hero-copy { width: 100%; max-width: 100%; } }

        .ss-kicker { display: inline-flex; align-items: center; gap: 9px; height: 28px; padding: 0 11px; margin-bottom: clamp(14px,3vw,20px); border: 1px solid rgba(238,246,255,0.10); background: rgba(1,6,16,0.45); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); color: rgba(232,242,255,0.65); font-family: "JetBrains Mono", monospace; font-size: clamp(7px,1.5vw,8px); font-weight: 700; letter-spacing: 0.18em; text-transform: uppercase; }
        ...
        @media (max-width: 400px) { .ss-actions-row { flex-direction: column; } .ss-btn { width: 100%; justify-content: center; } }
        ...
        .ss-status-row { display: grid; grid-template-columns: 88px minmax(0,1fr); gap: 8px; padding: 8px 11px; border-bottom: 1px solid rgba(255,255,255,0.04); align-items: center; }
        ...
        .ss-holo { position: relative; min-height: clamp(150px,22vw,214px); display: grid; place-items: center; overflow: hidden; border: 1px solid rgba(255,255,255,0.058); background: rgba(1,5,14,0.28); }
        ...
        .ss-holo-core strong { display: block; font-family: "Cormorant Garamond", Georgia, serif; font-size: clamp(24px,4vw,50px); font-style: italic; line-height: 0.92; color: #fff; }
        ...
        .ss-cortex-rail p { position: relative; margin: 12px 0 0; color: rgba(232,242,255,0.44); font-size: 11px; line-height: 1.6; }
        ...
        .ss-node code { display: block; margin-top: 7px; color: rgba(232,242,255,0.26); font-family: "JetBrains Mono", monospace; font-size: 8px; }
        ...
        .ss-footer-note { font-family: "JetBrains Mono", monospace; font-size: clamp(7px,1.5vw,8px); color: rgba(232,242,255,0.2); text-transform: uppercase; letter-spacing: 0.1em; }
      ...
      {/* THREE.js — only rendered once isMobile is known and is false */}
      {!isMobile && <CinematicScene />}

      {/* Mobile full-screen menu */}
      <div className={`ss-mobile-menu${menuOpen ? ' open' : ''}`}>
        <button className="ss-mobile-close" onClick={() => setMenuOpen(false)}>✕</button>
        {[
        ...
        <a href="https://github.com/NexTechArchitect/Sentinel-Insurance-Protocol"
          target="_blank" rel="noreferrer" className="ss-mobile-link"
          style={{ fontSize: 'clamp(14px,4vw,18px)', color: 'rgba(88,218,255,0.7)', transitionDelay: '225ms' }}
        >GitHub ↗</a>
      </div>
      ...
      <main className="ss-page">
        ...
        <nav className={`ss-nav${scrollY > 55 ? ' scrolled' : ''}`}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="ss-wallet-chip">
              {isConnected && <span className="ss-wallet-dot" />}
              <span>{isConnected && address ? `${address.slice(0,6)}...${address.slice(-4)}` : 'No wallet'}</span>
            </div>
            <button className="ss-ham" onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu">
              {[0,1,2].map(i => (
                <div key={i} style={{
                  width: 22, height: 1.5, background: '#fff', borderRadius: 1, transition: 'all 0.3s',
        ...
        <footer className="ss-footer">
          ...
          <span className="ss-footer-note">Sepolia · Verified</span>
        </footer>
      </main>
    </>
  );
